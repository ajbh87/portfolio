        ...
        function generateElementsObj() {
            const threshold = ((step) => {
                    ...
                    if (0 < step && step < 1) {
                        while(newStep <= 1) {
                            t.push(newStep);
                            newStep = __WEBPACK_IMPORTED_MODULE_1__saKnife_js__["a" /* default */].round(newStep + step, 1);
                        }
                    }
                    return t;
                })(0.1),
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_svg_js_dist_svg_js__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_svg_js_dist_svg_js___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1__node_modules_svg_js_dist_svg_js__);
...
class svgLine {
    constructor(options) {
        ...
        _this.svg = __WEBPACK_IMPORTED_MODULE_1__node_modules_svg_js_dist_svg_js___default()(options.svg);
        _this.svgPath = _this.svg.select(options.pathSelector).members[0];
    ...
    getRatios(triggerPoints) {
        ...
        let ratios = [],
            ys = [],
            points = _this.svgPath.array().value;
        triggerPoints.forEach((triggerPoint, index) => {
            let y = points[triggerPoint][1],
                newRatio = 0;
            ys.push(y);
    ...
    setRatios(ratios) {
        ...
        let points = _this.svgPath.array(),
            pointsArray = points.value,
            diffs = [],
            pointIndex = 0,
            triggerLengths = [];
        ...
        if (triggerPoints != null) {
            ratios.forEach((ratio, i) => {
                let y = pointsArray[triggerPoints[i]][1],
                    ratioDiff = ratio / oldRatios[i],
                    newY = 0;
                if (i > 0) {
                    y = y - pointsArray[triggerPoints[i - 1]][0];
                }
            ...
            triggerPoints.forEach((triggerIndex, index) => {
                const trigger = pointsArray[triggerIndex];
                ...
                debugger;
                for (pointIndex = 1; pointIndex < pointsArray.length; pointIndex++) {
                    point = pointsArray[pointIndex];
                    length = calculateLength(pointsArray[pointIndex - 1], point);

                    if (point[1] <= trigger[1]) {
                        if (lastTrigger != null) {
                            if (point[1] > pointsArray[lastTrigger][1]) {
                                point[1] = changeY(point);
                            }
                        }
                        else {
                            point[1] = changeY(point);
        ...
        debugger;
        _this.svgPath.plot(pointsArray);
        function calculateLength(pointSet1, pointSet2) {
            const lX = pointSet2[0] - pointSet1[0], 
                lY = pointSet2[1] - pointSet1[1];
